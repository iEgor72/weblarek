...
Хранит товары, полученные от сервера, и предоставляет доступ к каталогу.
...
Хранит выбранные товары и выполняет расчёты, относящиеся к корзине. Повторное добавление уже выбранного товара игнорируется.
...
- `isValid(): boolean` — сообщает, заполнены ли все поля;
...
- `getProducts(): Promise<IProduct[]>` — получает список товаров через `GET /product/`;
